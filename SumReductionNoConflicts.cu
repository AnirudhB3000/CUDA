...
//modulo operation removed, bank conflicts avoided
#include <cstdlib>
#include <iostream>
#include <vector>
#include <algorithm>
...
__global__ void sumReduction (int *v, int *v_r) {
  ...
  for (int s = blockDim.x / 2; s > 0; s >>= 1) { //reduce the number of threads in each iteration, bottom up approach is faster and avoids memory bank conflicts
    if (threadIdx.x < s) {
